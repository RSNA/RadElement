<?php
/*
	/editor/element-value-codes.php
*/
	...
	// Get parameters
	extract ($_REQUEST);	// $elementID & $valueCode
	$key = make_key();
	$codeURL = array (
	...
	if (! isset ($elementID) || ! ctype_digit($elementID) || ! isset ($valueCode)) {
		print "Invalid elementID or valueCode\n";
		exit (1);
	}
	...
	if (strcmp ($action, "delete") == 0) {
		...
		header ("Location: element-value-codes.php?elementID=$elementID&valueCode=$valueCode");
		exit;
	}
	...
	if (strcmp ($action, "add") == 0) {
		foreach ($new as $n => $on) {
			...
			mysql_query ("INSERT INTO CodeRef (codeID, elementID, valueCode)
							VALUES ($codeID, $elementID, '$valueCode')");
		}
		header ("Location: element-value-codes.php?elementID=$elementID&valueCode=$valueCode&search=$search");
		exit;
	}
	...
	print "<html>
<head>
<title>Codes - Element RDE$elementID / Value $valueCode</title>
</head>
<body>
<h2>Codes - Element RDE$elementID / Value $valueCode = $name</h2>
";
 ...
 <a href=element-value-codes.php?elementID=$elementID&valueCode=$valueCode&codeRefID=$codeRefID&action=delete&key=$key>[delete]</a></li>
";
	}
	print "</ul>\n";
...
<form method=POST action=element-value-codes.php>
<input type=hidden name=action value=search>
<input type=hidden name=elementID value=$elementID>
<input type=hidden name=valueCode value=\"$valueCode\">
...
";
 ...
 $display (<a target=code href=$url>$system::$code</a>)</p>\n";
	...
	// Print footer
	print "<p>&nbsp;</p>
<p><a href=element-values.php?id=$elementID>Go back to Edit Element Values</a></p>
